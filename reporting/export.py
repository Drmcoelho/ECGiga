...
def to_md(rep: dict) -> str:
    ...
    if ivr:
        ...
        lines.append(f"- batimentos usados: {rep.get('intervals_refined',{}).get('beats_used')} / {rep.get('intervals_refined',{}).get('beats_total')}")
        lines.append("")
    if axis:
...
def export(report_json: str, out_md: str = None, out_html: str = None, overlay_path: str = None) -> tuple[str,str]:
    rep = json.loads(Path(report_json).read_text(encoding='utf-8'))
    md = to_md(rep)
    if out_md:
        Path(out_md).write_text(md, encoding='utf-8')
