...
def _rs_amplitudes(sig: np.ndarray, r_idx: int, fs: float) -> Tuple[float,float]:
    ...
    basew0 = max(0, r_idx - int(0.12*fs)); basew1 = max(0, r_idx - int(0.08*fs))
    base = float(np.median(sig[basew0:basew1])) if (basew1-basew0)>3 else float(np.median(sig[max(0,r_idx-10):r_idx]))
    seg = sig[i0:i1+1]
    if seg.size < 3: return 0.0, 0.0
...
def detect_blocks(image_path: str, layout: str = "3x4", anchor_lead: str = "II") -> Dict:
    ...
    # scoring simples
    brd_score = 0.0; bre_score = 0.0
    if qrs_ms and qrs_ms >= 120: 
        brd_score += 1.0; bre_score += 1.0  # ambos exigem QRS largo
    v1 = feats.get("V1", {}); v2 = feats.get("V2", {}); v6 = feats.get("V6", {}); leadI = feats.get("I", {})
    # BRD: em V1/V2 picos múltiplos (RSR') ou RS_ratio alto; e em I/V6 S proeminente
    if max(v1.get("peaks",0), v2.get("peaks",0)) >= 2.0 or max(v1.get("RS_ratio",0), v2.get("RS_ratio",0)) >= 1.5:
        brd_score += 1.0
    ...
    if max(leadI.get("peaks",0), v6.get("peaks",0)) >= 2.0 and (v1.get("S",0) > 1.2*max(v1.get("R",1e-9), 1e-9)):
        bre_score += 1.5
