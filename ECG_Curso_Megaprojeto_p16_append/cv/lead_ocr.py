...
def _render_templates(
    font: Optional[ImageFont.ImageFont] = None, sizes=(18, 22, 26)
    ...
    for sz in sizes:
        ...
        for lab in LABELS:
            ...
            w = max(12, bbox[2] - bbox[0] + 6)
            h = max(12, bbox[3] - bbox[1] + 6)
...
def _match_template(gray: np.ndarray, templ: np.ndarray) -> float:
    ...
    if gray is None or templ is None:
        return 0.0
    if gray.ndim != 2:
        gray = _pil_to_gray(gray)
    if templ.ndim != 2:
        templ = _pil_to_gray(templ)
    gh, gw = gray.shape
    th, tw = templ.shape
    if gh < th or gw < tw:
        return 0.0
...
def _ocr_text(gray_crop: np.ndarray) -> str:
    ...
    try:
        txt = pytesseract.image_to_string(
            gray_crop, config="--psm 7 -c tessedit_char_whitelist=IVaLRF123456"
        )
        return (txt or "").strip()
    except Exception:
        return ""
...
def detect_labels_per_box(gray: np.ndarray, boxes: List[Tuple[int, int, int, int]]) -> List[Dict]:
    ...
    for x0, y0, x1, y1 in boxes:
        ...
        if crop.size == 0:
            out.append({"bbox": (x0, y0, x1, y1), "label": None, "score": 0.0})
            continue
...
def score_layout(labels_detected: List[Dict], expected_sequence: List[str]) -> float:
    ...
    ok = 0
    total = min(len(labs), len(expected_sequence))
