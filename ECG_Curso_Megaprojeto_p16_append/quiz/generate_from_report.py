...
def _add_q(
    ...
    choices = [{"text": correct, "is_correct": True, "explanation": exp}] + [
        {"text": w, "is_correct": False, "explanation": "Incorreto no contexto do laudo."}
        for w in wrong
    ]
    questions.append({"prompt": prompt, "choices": choices, **({"image": image} if image else {})})


def quiz_from_report_illustrated(
    report_json: str,
    out_json: str,
    ...
    qrs = med.get("QRS_ms")
    qtc = med.get("QTc_B") or med.get("QTc_ms")
    if qrs is not None:
        _add_q(
            qs,
            "Quanto à largura do QRS, a melhor interpretação é:",
            f"QRS ≈ {qrs:.0f} ms (≥120 ms sugere bloqueio completo).",
            [
                "QRS inerente ao eixo (não depende de condução).",
                "QRS não é interpretável em derivação de membros.",
            ],
    ...
    if qtc is not None:
        _add_q(
            qs,
            "Sobre o QT corrigido (QTc), assinale a correta:",
    ...
    if ang is not None:
        _add_q(
            qs,
            "Qual a melhor descrição do eixo frontal?",
    ...
    if hv:
        ...
        cp = hv.get("cornell_product_mm_ms") or hv.get("cornell_mV")
        _add_q(
            qs,
            "Sobre critérios de HVE, assinale a correta:",
