        ...
        choices.append(
            {
                "text": w,
                "is_correct": False,
                "explanation": "Alternativa incorreta dadas as evidências do laudo.",
            }
        )
...
def quiz_from_report(rep: Dict, n_max: int = 10) -> Dict:
        ...
        if qrs is not None:
            qs.append(
                _mk_question(
                    ...
                    [
                        "QRS estreito por definição (≤ 90 ms).",
                        "QRS não pode ser estimado a partir de laudos automáticos.",
                    ],
        ...
        if qtc is not None:
            qs.append(
                _mk_question(
                    ...
                    [
                        "QTc ≤ 360 ms é sempre patológico.",
                        "QTc > 500 ms só ocorre em taquiarritmias ventriculares.",
                    ],
        ...
        if ang is not None:
            qs.append(
                _mk_question(
                    ...
                    f"Eixo ≈ {ang:.0f}° — {lab}.",
                    [
                        "Eixo indeterminável por ECG de 12 derivações.",
                        "Eixo depende apenas de V1 e V6.",
                    ],
    ...
    if hv:
        sok = hv.get("sokolow_lyon_mm")
        corn = hv.get("cornell_product_mm_ms") or hv.get("cornell_mV")
        qs.append(
            _mk_question(
                "Sobre hipertrofia ventricular esquerda (HVE):",
                f"Sokolow-Lyon ≈ {sok:.1f} mm e Cornell product ≈ {corn} (ver thresholds).",
                [
                    "Critérios de HVE não podem ser estimados a partir do ECG.",
                    "Cornell product usa apenas V5/V6.",
                ],
