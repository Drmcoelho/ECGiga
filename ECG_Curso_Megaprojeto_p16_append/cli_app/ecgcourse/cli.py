...
def analyze_values(
    ...
    intervals_refined_flag: bool = typer.Option(
        False, "--intervals-refined", help="Intervalos refinados (multi-evidência)"
    ),
    axis_flag: bool = typer.Option(False, "--axis", help="Calcular eixo frontal I/aVF"),
    schema_v5: bool = typer.Option(
        True, "--schema-v5/--schema-v4-off", help="Emitir laudo no schema v0.5"
    ),
    ...
    if auto_grid:
        try:
            ...
            if auto_leads:
                ...
                layout_det = choose_layout(
                    _np.asarray(Image.open(p).convert("L")), {"3x4": [d["bbox"] for d in seg_leads]}
                )
            ...
            if rpeaks_lead:
                ...
                if rpeaks_lead in _lab2box:
                    ...
                    _pxmm = (grid.get("px_small_x") if grid else None) or (
                        grid.get("px_small_y") if grid else None
                    )
                    ...
                    if intervals_refined_flag:
                        ...
                        intervals_refined_out = intervals_refined_from_trace(
                            _trace, rpeaks_out.get("peaks_idx") or [], _pxsec
                        )
                    if axis_flag:
                        ...
                        axis_out = frontal_axis_from_image(
                            ...
                            {rpeaks_lead: rpeaks_out.get("peaks_idx", [])},
                            {rpeaks_lead: _pxsec},
                        )
...
def cv_intervals_refined(
    ...
    if dump_json:
        ...
        m = iv["median"]
        print(
            f"[refined] PR {m.get('PR_ms')} ms | QRS {m.get('QRS_ms')} ms | QT {m.get('QT_ms')} ms | QTcB {m.get('QTc_B')} ms | QTcF {m.get('QTc_F')} ms"
        )
    return out
...
def cv_axis(
    ...
    axis = frontal_axis_from_image(
        gray,
        {"I": lab2box.get("I"), "aVF": lab2box.get("aVF")},
        {"I": rI, "aVF": rF},
        {"I": fsI, "aVF": fsF},
    )
    if dump_json:
        print(_json.dumps(axis, ensure_ascii=False, indent=2))
    else:
        print(
            f"Eixo: {axis['angle_deg']:.1f}° — {axis['label']} (I={axis['amps'].get('I'):.2f}, aVF={axis['amps'].get('aVF'):.2f})"
        )
    return axis
...
def quiz_adaptive(
    report_json: str = typer.Argument(..., help="Laudo JSON (v0.4+ com intervals/axis)"),
    out_json: str = typer.Option(None, "--out", help="Arquivo de saída (JSON)"),
...
def cv_overlay(
    ...
    trace = smooth_signal(extract_trace_centerline(crop), win=11)
    pxmm = estimate_grid_period_px(_np.asarray(_Image.open(image_path).convert("RGB"))).get(
        "px_small_x"
    ) or estimate_grid_period_px(_np.asarray(_Image.open(image_path).convert("RGB"))).get(
        "px_small_y"
    )
    ...
    outp = draw_overlay(
        image_path, lab2box, {lead: rdet["peaks_idx"]}, iv, out_path, highlight_lead=lead
    )
...
def export_report(
    report_json: str = typer.Argument(..., help="Laudo JSON (v0.5)"),
    out_md: str = typer.Option(None, "--md"),
    out_html: str = typer.Option(None, "--html"),
    robust: bool = typer.Option(
        True, "--robust/--no-robust", help="Aplica estatística robusta (MAD)"
    ),
):
...
def cv_axis_hex(
    ...
    if dump_json:
        print(_json.dumps(ax, ensure_ascii=False, indent=2))
    else:
        print(
            f"Eixo (hex): {ax['angle_deg']:.1f}° — {ax['label']} | usados={','.join(ax['leads_used'])} (anchor={ax['anchor']})"
        )
    return ax
...
def rhythm_analyze(
    ...
    if json_out:
        ...
        print(
            f"Ritmo: {rep['label']} | HR≈{rep['features'].get('hr_bpm')} bpm | SDNN≈{rep['features'].get('sdnn_ms')} ms"
        )
    return rep
...
def precordials_transition(
    ...
    if json_out:
        ...
        print(f"Transição em: {rep.get('transition_at')} — padrão {rep.get('pattern')}")
    return rep
...
def checklist_lvh(
    ...
    if json_out:
        ...
        sl = rep["sokolow_lyon_mm"]
        cm = rep["cornell_mm"]
        thr = rep["cornell_threshold_mm"]
        print(
            f"Sokolow-Lyon={sl:.1f} mm | Cornell={cm:.1f} mm (thr={thr:.1f}) | LVH: S-L={rep['LVH_sokolow']} Cornell={rep['LVH_cornell']}"
        )
    return rep
...
def blocks_detect(
    ...
    if json_out:
        ...
        print(f"{rep['label']} | QRS≈{rep.get('qrs_ms')} ms | score={rep['score']}")
    return rep
...
def hypertrophy_extended_cmd(
    ...
    if json_out:
        ...
        print(
            f"Sokolow={rep['sokolow_lyon_mm']:.1f} mm | CornellProd={rep['cornell_product_mm_ms']:.0f} (thr={rep['thresholds']['cornell_product_mm_ms']:.0f})"
        )
    return rep


@quiz.command("from-report")
def quiz_from_report_cmd(
    report_json: str = typer.Argument(..., help="Laudo JSON v0.5"),
...
def quiz_from_report_illustrated_cmd(
    report_json: str = typer.Argument(..., help="Laudo JSON v0.5"),
    ...
    out = quiz_from_report_illustrated(
        report_json,
        out_quiz,
        image_path=image_path,
        overlay_json=overlay_json,
        overlay_out="reports/overlays/auto_overlay.png" if (image_path and overlay_json) else None,
    )
...
def perf_report_html(
    input_glob: str = typer.Argument(
        "reports/annotations_eval*.json", help="Glob de JSONs de comparação"
    ),
