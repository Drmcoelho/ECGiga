...
def analyze_values(
    ...
    intervals_refined_flag: bool = typer.Option(False, "--intervals-refined", help="Intervalos refinados (multi-evidência)"),
    axis_flag: bool = typer.Option(False, "--axis", help="Calcular eixo frontal I/aVF"),
    schema_v5: bool = typer.Option(True, "--schema-v5/--schema-v4-off", help="Emitir laudo no schema v0.5"),
...
def ingest_image(
    ...
    report_obj = {
        ...
        "intervals_refined": (intervals_refined_out if intervals_refined_flag else None),
        "axis": (axis_out if axis_flag else None),
        "version": ("0.5.0" if schema_v5 else ("0.4.0" if schema_v4 else ("0.3.0" if schema_v3 else ("0.2.0" if schema_v2 else "0.1.0"))))
    }
...
@cv_app.command("intervals-refined")
def cv_intervals_refined(image_path: str = typer.Argument(..., help="PNG/JPG"),
                         layout: str = typer.Option("3x4", "--layout"),
                         lead: str = typer.Option("II", "--lead"),
                         speed_mm_s: float = typer.Option(25.0, "--speed"),
                         dump_json: bool = typer.Option(False, "--json")):
    """Intervalos (PR/QRS/QT/QTc) via multi-evidência ao redor de R-peaks robustos."""
    import json as _json, numpy as _np
    from PIL import Image
    from cv.segmentation import find_content_bbox
    from cv.segmentation_ext import segment_layout
    ...
    out = {"lead_used": lead, "intervals_refined": iv}
    if dump_json: print(_json.dumps(out, ensure_ascii=False, indent=2))
    else:
        m = iv["median"]
        print(f"[refined] PR {m.get('PR_ms')} ms | QRS {m.get('QRS_ms')} ms | QT {m.get('QT_ms')} ms | QTcB {m.get('QTc_B')} ms | QTcF {m.get('QTc_F')} ms")
    return out


@cv_app.command("axis")
def cv_axis(image_path: str = typer.Argument(..., help="PNG/JPG"),
            layout: str = typer.Option("3x4", "--layout"),
            speed_mm_s: float = typer.Option(25.0, "--speed"),
            dump_json: bool = typer.Option(False, "--json")):
    """Calcula o eixo frontal I/aVF a partir da imagem (amplitude líquida do QRS)."""
    import json as _json, numpy as _np
    from PIL import Image
    from cv.segmentation import find_content_bbox
    from cv.segmentation_ext import segment_layout
    ...
    lab2box = {d["lead"]: d["bbox"] for d in seg}
    def rpeaks_for(lead):
        if lead not in lab2box: return [], 250.0
        x0,y0,x1,y1 = lab2box[lead]
        crop = gray[y0:y1, x0:x1]
        trace = smooth_signal(extract_trace_centerline(crop), win=11)
        pxmm = (estimate_grid_period_px(_np.asarray(Image.open(image_path).convert("RGB"))).get("px_small_x")
        ...
        pxsec = estimate_px_per_sec(pxmm, speed_mm_per_sec=speed_mm_s) or 250.0
        rdet = pan_tompkins_like(trace, pxsec)
        return rdet["peaks_idx"], pxsec
    rI, fsI = rpeaks_for("I")
    rF, fsF = rpeaks_for("aVF")
    axis = frontal_axis_from_image(gray, {"I": lab2box.get("I"), "aVF": lab2box.get("aVF")}, {"I": rI, "aVF": rF}, {"I": fsI, "aVF": fsF})
    if dump_json: print(_json.dumps(axis, ensure_ascii=False, indent=2))
    else: print(f"Eixo: {axis['angle_deg']:.1f}° — {axis['label']} (I={axis['amps'].get('I'):.2f}, aVF={axis['amps'].get('aVF'):.2f})")
    return axis
...
def quiz_adaptive(report_json: str = typer.Argument(..., help="Laudo JSON (v0.4+ com intervals/axis)"),
    ...
    rep = _json.load(open(report_json,"r",encoding="utf-8"))
    q = build_adaptive_quiz(rep, n_questions=n_questions)
    if out_json:
        _json.dump(q, open(out_json,"w",encoding="utf-8"), ensure_ascii=False, indent=2)
        print(f"Quiz adaptativo salvo em {out_json}")
    else:
        print(_json.dumps(q, ensure_ascii=False, indent=2))


@cv_app.command("overlay")
def cv_overlay(image_path: str = typer.Argument(..., help="PNG/JPG"),
               lead: str = typer.Option("II", "--lead"),
               out_path: str = typer.Option("reports/overlays/overlay.png", "--out")):
    """Gera overlay com R-peaks e janelas QRS no lead escolhido."""
    import os as _os
    ...
    from cv.rpeaks_robust import pan_tompkins_like
    from cv.intervals_refined import intervals_refined_from_trace
    from cv.overlay import draw_overlay
    arr = _np.asarray(_Image.open(image_path).convert("L"))
    ...
    trace = smooth_signal(extract_trace_centerline(crop), win=11)
    pxmm = (estimate_grid_period_px(_np.asarray(_Image.open(image_path).convert("RGB"))).get("px_small_x")
            or estimate_grid_period_px(_np.asarray(_Image.open(image_path).convert("RGB"))).get("px_small_y"))
...
def export_report(report_json: str = typer.Argument(..., help="Laudo JSON (v0.5)"),
                  ...
                  robust: bool = typer.Option(True, "--robust/--no-robust", help="Aplica estatística robusta (MAD)")):
    """Exporta laudo para Markdown/HTML; opcionalmente aplica medianas robustas (MAD) antes de exportar."""
    import json as _json
    from reporting.export import export as _export
    if robust:
        rep = _json.load(open(report_json,"r",encoding="utf-8"))
        iv = rep.get("intervals_refined")
        if iv:
...
@cv_app.command("axis-hex")
def cv_axis_hex(image_path: str = typer.Argument(..., help="PNG/JPG"),
                layout: str = typer.Option("3x4", "--layout"),
                anchor: str = typer.Option("II", "--anchor"),
                dump_json: bool = typer.Option(False, "--json")):
    """Eixo frontal pelo sistema hexaxial (I, II, III, aVR, aVL, aVF)."""
    import json as _json
    from cv.axis_hexaxial import hexaxial_axis_from_image
    ax = hexaxial_axis_from_image(image_path, layout=layout, anchor_lead=anchor)
    if dump_json: print(_json.dumps(ax, ensure_ascii=False, indent=2))
    else: print(f"Eixo (hex): {ax['angle_deg']:.1f}° — {ax['label']} | usados={','.join(ax['leads_used'])} (anchor={ax['anchor']})")
    return ax
...
def rhythm_analyze(image_path: str = typer.Argument(..., help="PNG/JPG"),
    ...
    """Análise de ritmo: HR/SDNN/CV-RR e rótulo (sinusal/irregular/indeterminado)."""
    import json as _json
    from cv.rhythm import analyze_rhythm
    rep = analyze_rhythm(image_path, lead=lead, layout=layout)
    if json_out: print(_json.dumps(rep, ensure_ascii=False, indent=2))
    else: print(f"Ritmo: {rep['label']} | HR≈{rep['features'].get('hr_bpm')} bpm | SDNN≈{rep['features'].get('sdnn_ms')} ms")
    return rep
...
def precordials_transition(image_path: str = typer.Argument(..., help="PNG/JPG"),
                           ...
                           anchor: str = typer.Option("II", "--anchor"),
                           json_out: bool = typer.Option(False, "--json")):
    """Transição R/S (V1–V6): razão R/S por derivação e ponto de transição."""
    import json as _json
    from cv.precordial_transition import analyze_transition
    rep = analyze_transition(image_path, layout=layout, anchor_lead=anchor)
    if json_out: print(_json.dumps(rep, ensure_ascii=False, indent=2))
    else: print(f"Transição em: {rep.get('transition_at')} — padrão {rep.get('pattern')}")
    return rep
...
def checklist_lvh(image_path: str = typer.Argument(..., help="PNG/JPG"),
                  ...
                  layout: str = typer.Option("3x4", "--layout"),
                  json_out: bool = typer.Option(False, "--json")):
    """HVE (Sokolow-Lyon/Cornell) a partir da imagem (usa calibração da grade)."""
    import json as _json
    from cv.lvh_checklist import lvh_checklist
    rep = lvh_checklist(image_path, sex=sex, layout=layout)
    if json_out: print(_json.dumps(rep, ensure_ascii=False, indent=2))
    else:
...
@blocks.command("detect")
def blocks_detect(image_path: str = typer.Argument(..., help="PNG/JPG"),
                  layout: str = typer.Option("3x4", "--layout"),
                  anchor: str = typer.Option("II", "--anchor"),
                  json_out: bool = typer.Option(False, "--json")):
    """BRD/BRE heurístico: QRS (ms) + morfologia V1/V2 e I/V6."""
    import json as _json
    from cv.blocks import detect_blocks
    rep = detect_blocks(image_path, layout=layout, anchor_lead=anchor)
    if json_out: print(_json.dumps(rep, ensure_ascii=False, indent=2))
    else: print(f"{rep['label']} | QRS≈{rep.get('qrs_ms')} ms | score={rep['score']}")
    return rep


@app.command("hypertrophy")
def hypertrophy(): ...


@hypertrophy.command("extended")
def hypertrophy_extended_cmd(image_path: str = typer.Argument(..., help="PNG/JPG"),
                             sex: str = typer.Option("male","--sex"),
                             layout: str = typer.Option("3x4","--layout"),
                             json_out: bool = typer.Option(False,"--json")):
    """HVE estendido: Sokolow-Lyon e Cornell product (mm·ms)."""
    import json as _json
    from cv.hypertrophy_extended import hypertrophy_extended
    rep = hypertrophy_extended(image_path, sex=sex, layout=layout)
    if json_out: print(_json.dumps(rep, ensure_ascii=False, indent=2))
    else: 
        print(f"Sokolow={rep['sokolow_lyon_mm']:.1f} mm | CornellProd={rep['cornell_product_mm_ms']:.0f} (thr={rep['thresholds']['cornell_product_mm_ms']:.0f})")
    return rep


@quiz.command("from-report")
def quiz_from_report_cmd(report_json: str = typer.Argument(..., help="Laudo JSON v0.5"),
                         out_json: str = typer.Option(None, "--out")):
    """Gera um conjunto de MCQs coerente com o laudo atual."""
    import json as _json
    from quiz.from_report import quiz_from_report
    rep = _json.load(open(report_json,"r",encoding="utf-8"))
    qs = quiz_from_report(rep, n_max=10)
    if out_json:
        Path(out_json).parent.mkdir(parents=True, exist_ok=True)
        Path(out_json).write_text(_json.dumps(qs, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"Quiz salvo em {out_json}")
    else:
        print(_json.dumps(qs, ensure_ascii=False, indent=2))
    return qs
...
@web_app.command("case-player")
def web_case_player(port: int = typer.Option(8013, "--port")):
    """Atalho para servir o Case Player estático."""
    import os, http.server, socketserver
    os.chdir("web_app/case_player")
    Handler = http.server.SimpleHTTPRequestHandler
    with socketserver.TCPServer(("", port), Handler) as httpd:
        print(f"Case Player em http://127.0.0.1:{port}")
        httpd.serve_forever()
...
@auto.command("quiz-illustrated")
def quiz_from_report_illustrated_cmd(report_json: str = typer.Argument(..., help="Laudo JSON v0.5"),
                                     image_path: str = typer.Option(None, "--image", help="Imagem do ECG"),
                                     overlay_json: str = typer.Option(None, "--overlay", help="annotations.json opcional"),
                                     out_quiz: str = typer.Option("quizzes/auto.quiz.json", "--out-quiz"),
                                     out_html: str = typer.Option(None, "--out-html", help="Se definido, gera HTML standalone")):
    """Gera MCQs ilustrados a partir de um laudo (com overlay opcional) e, opcionalmente, exporta HTML único."""
    from quiz.generate_from_report import quiz_from_report_illustrated
    out = quiz_from_report_illustrated(report_json, out_quiz, image_path=image_path,
                                       overlay_json=overlay_json, overlay_out="reports/overlays/auto_overlay.png" if (image_path and overlay_json) else None)
    print(f"Quiz salvo em {out}")
    if out_html:
        from quiz.html import render_html
        render_html.callback(quiz_json=out, out_html=out_html, base_dir=".")  # type: ignore


@app.command("perf")
def perf(): ...
@perf.command("report-html")
def perf_report_html(input_glob: str = typer.Argument("reports/annotations_eval*.json", help="Glob de JSONs de comparação"),
                     out_html: str = typer.Argument("reports/performance.html", help="Saída HTML")):
    """Gera um relatório HTML simples a partir dos JSONs de avaliação de anotações."""
    from reports.performance_html import build_report
    p = build_report(input_glob, out_html)
    print(f"Relatório em {p}")
