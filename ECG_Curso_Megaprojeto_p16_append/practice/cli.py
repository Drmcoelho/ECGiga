...
@app.command("build")
def build_practice(
    reports_dir: str = typer.Argument("reports", help="Pasta com *.report.json"),
    ...
    for p in rep_dir.glob("*.json"):
        try:
            rep = _load_json(p)
            meta = rep.get("meta", {})
            item = {"name": Path(meta.get("src", p.name)).stem, "report": rep}
    ...
    (out / "_index.json").write_text(
        json.dumps({"cases": items}, ensure_ascii=False, indent=2), encoding="utf-8"
    )
...
@app.command("quiz")
def run_quiz(
    quiz_json: str = typer.Argument(..., help="Arquivo quiz *.json"),
