...
def make_overlay_figure(img, seg=None, title="Preview do ECG"):
    ...
    if seg and seg.get("leads"):
        for ld in seg["leads"]:
            x0, y0, x1, y1 = ld["bbox"]
            shapes.append(dict(type="rect", x0=x0, y0=h - y1, x1=x1, y1=h - y0, line=dict(width=2, color="cyan")))
            annotations.append(dict(x=(x0 + x1) / 2, y=h - y1 + 15, text=ld.get("lead", "?"), showarrow=False, bgcolor="rgba(255,255,255,0.5)"))
    fig.update_layout(title=title, xaxis=dict(visible=False, range=[0, w]), yaxis=dict(visible=False, range=[0, h], scaleanchor="x", scaleratio=1), shapes=shapes, annotations=annotations, margin=dict(l=0, r=0, t=40, b=0))
    return fig
...
analysis_controls = dbc.Card([
    ...
    dbc.CardBody([
        dcc.Upload(id='upload-ecg', children=html.Div(['Arraste e solte ou ', html.A('selecione um arquivo')]), multiple=False, style={'width':'100%','height':'60px','lineHeight':'60px','borderWidth':'1px','borderStyle':'dashed','borderRadius':'5px','textAlign':'center','marginBottom':'10px'}),
        dcc.Textarea(id="upload-meta", placeholder="Cole o sidecar META (JSON) opcional...", style={"width":"100%","height":"80px"}),
        dbc.Row([
            dbc.Col(dcc.Checklist(id='ops', options=[{'label': 'Deskew', 'value': 'deskew'}, {'label': 'Normalize', 'value': 'normalize'}], value=[], inline=True), width='auto'),
            dbc.Col(dbc.InputGroup([dbc.InputGroupText("Layout"), dcc.Dropdown(id='layout-select', options=[{'label': '3x4', 'value': '3x4'}, {'label': '6x2', 'value': '6x2'}], value='3x4', clearable=False)], size="sm"), width='auto')
        ], align="center", className="mb-2"),
        html.Div([
            dbc.Button("Processar Imagem", id="btn-process", n_clicks=0, color="primary"),
            dbc.Spinner(html.Div(id="spinner-dummy"), color="primary")
        ], style={"display": "flex", "gap": "15px", "alignItems": "center"}, className="mb-2"),
        dbc.Progress(id="progress-bar", style={"height": "20px"}, className="mb-2"),
        html.Div(id="upload-summary", style={"whiteSpace":"pre-wrap", "maxHeight": "250px", "overflowY": "auto", "border": "1px solid #ccc", "padding": "10px"}),
    ])
])
...
layout = dbc.Container([
    dbc.Row([
        ...
        dbc.Col(dcc.Graph(id="overlay-graph", figure=go.Figure().update_layout(title="Aguardando imagem...")), md=8)
    ])
], fluid=True)
