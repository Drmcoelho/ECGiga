...
def create_comparison_card(id_prefix):
    ...
    return dbc.Card([
        ...
        dbc.CardBody([
            dcc.Upload(id=f'upload-ecg-{id_prefix}', children=['Arraste ou ', html.A('selecione')], style={'width':'100%','height':'60px','lineHeight':'60px','borderWidth':'1px','borderStyle':'dashed','borderRadius':'5px','textAlign':'center','marginBottom':'10px'}),
...
layout = dbc.Container([
    ...
    dbc.Row([
        ...
        dbc.Col(dbc.Card([
            ...
            dbc.CardBody([
                dbc.Button("Comparar ECGs", id="btn-compare", color="primary", className="w-100 mb-3"),
                dbc.Progress(id="progress-bar-compare", style={"height": "20px"}, className="mb-3"),
...
def compare_ecgs_callback(set_progress, n_clicks, content_a, filename_a, content_b, filename_b):
    ...
    total_steps = 10
    report_a, summary_a, fig_a = run_ecg_pipeline(set_progress, content_a, filename_a, 0, total_steps)
    report_b, summary_b, fig_b = run_ecg_pipeline(set_progress, content_b, filename_b, 5, total_steps)

    # Gerar a tabela de comparação
    header = [html.Thead(html.Tr([html.Th("Parâmetro"), html.Th("ECG A"), html.Th("ECG B"), html.Th("Delta")]))]
