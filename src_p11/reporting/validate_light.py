...
def validate_light(rep: Dict) -> List[str]:
    ...
    # chaves mínimas
    if "meta" not in rep:
        errs.append("faltando: meta")
    if "version" not in rep:
        errs.append("faltando: version")
    ...
    for axk in ("axis", "axis_hex"):
        ax = rep.get(axk)
        if ax:
            if not _is_num(ax.get("angle_deg")):
                errs.append(f"{axk}.angle_deg deve ser numérico")
            if "label" not in ax:
                errs.append(f"{axk}.label ausente")
    # flags string
    if "flags" in rep:
        if not isinstance(rep["flags"], list):
            errs.append("flags deve ser lista")
        else:
            for f in rep["flags"]:
                if not isinstance(f, str):
                    errs.append("flags.* deve ser string")
    return errs
