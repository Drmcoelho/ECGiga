    ...
    try:
        ...
        if auto_grid:
            try:
                ...
                seg = {
                    "content_bbox": bbox.tolist() if hasattr(bbox, "tolist") else bbox,
                    "leads": seg_leads,
                }
                        ...
                        if rpeaks_lead in lab2box:
                            ...
                            if rpeaks_robust:
                                try:
                                    ...
                                    rpeaks_out = {
                                        "peaks_idx": rob["peaks_idx"],
                                        "method": "pan_tompkins_like",
                                        "lead_used": rpeaks_lead,
                                    }
                                    capabilities.append("rpeaks_robust")
                                except ImportError:
                                    flags.append("rpeaks_robust_unavailable")
                            else:
                                rpeaks_out = detect_rpeaks_from_trace(
                                    trace, px_per_sec=pxsec, zthr=2.0
                                )
                            ...
                            if intervals and rpeaks_out.get("peaks_idx"):
                                try:
                                    ...
                                    intervals_out = intervals_from_trace(
                                        trace, rpeaks_out["peaks_idx"], pxsec
                                    )
        ...
        angle, axis_label = (
            _axis_from_I_aVF(lead_i, avf)
            if (lead_i is not None and avf is not None)
            else (None, None)
        )
        qb, qf = (None, None)
