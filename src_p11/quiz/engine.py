...
ITEMS = [
    {
        "id": "axis_left_1",
        "tag": "axis_left",
        "prompt": "O eixo elétrico frontal está desviado para a esquerda quando:",
        "choices": [
            {
                "text": "Ângulo entre -30° e -90°",
                "ok": True,
                "exp": "Definição clássica: -30° a -90°.",
            },
            {"text": "Ângulo > +90°", "ok": False, "exp": "Isso é desvio para a direita."},
            {
                "text": "Ângulo entre -15° e +75°",
                "ok": False,
                "exp": "Faixa aproximada de normalidade.",
            },
    ...
    },
    {
        "id": "axis_right_1",
        "tag": "axis_right",
        "prompt": "Selecione o intervalo que define desvio do eixo para a direita:",
        "choices": [
    ...
    },
    {
        "id": "qrs_wide_1",
        "tag": "qrs_wide",
        "prompt": "QRS é considerado largo quando:",
        "choices": [
    ...
    },
    {
        "id": "pr_long_1",
        "tag": "pr_long",
        "prompt": "Bloqueio AV de 1º grau é definido por:",
        "choices": [
    ...
    },
    {
        "id": "qtc_long_1",
        "tag": "qtc_long",
        "prompt": "QTc prolongado de alto risco (genérico) é:",
        "choices": [
...
def infer_gaps_from_report(report: Dict) -> List[str]:
    ...
    if qrs is not None and qrs >= 120:
        tags.append("qrs_wide")
    if pr is not None and pr > 200:
        tags.append("pr_long")
    if qtc is not None and qtc > 480:
        tags.append("qtc_long")
    if "esquerda" in axis:
        tags.append("axis_left")
    if "direita" in axis:
        tags.append("axis_right")
...
def build_adaptive_quiz(report: Dict, n_questions: int = 6, seed: int = 123) -> Dict:
    ...
    sel = (
        ...
        else (pool + random.sample(ITEMS, k=max(0, n_questions - len(pool))))
    )
    questions = []
    for it in sel:
        ch = it["choices"][:]
        ...
        questions.append(
            {
                ...
                "prompt": it["prompt"],
                "choices": [
                    {"id": i, "text": c["text"], "is_correct": c["ok"], "explanation": c["exp"]}
