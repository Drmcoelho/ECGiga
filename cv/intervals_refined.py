...
def _grad(x): return np.diff(x, prepend=x[:1])
...
def find_onset_offset_refined(y_px: np.ndarray, r_idx: int, fs: float) -> Tuple[int,int]:
    ...
    y = _smooth(y, 7)
    g = _grad(y); gabs = _norm01(np.abs(g))
    E = _energy(y, win=int(0.04*fs))
    pre = int(0.16*fs)
    pos = int(0.18*fs)
    i0 = max(0, r_idx-pre); i1 = min(len(y)-1, r_idx+pos)
    gw = gabs[i0:i1+1]; Ew = E[i0:i1+1]
    ...
    if qrs_ms < 60:  offset = min(len(y)-1, onset + int(0.08*fs))
    if qrs_ms > 200: offset = onset + int(0.20*fs)
...
def intervals_refined_from_trace(y_px: np.ndarray, r_peaks: List[int], px_per_sec: float) -> Dict:
    ...
    for i in range(len(r_peaks)):
        po = pons[i]; on=onsets[i]; off=offsets[i]; te=tend[i]
        if po is not None and on is not None: PR.append((on-po)*1000.0/fs)
        if on is not None and off is not None: QRS.append((off-on)*1000.0/fs)
        if on is not None and te  is not None: QT.append((te-on)*1000.0/fs)
    ...
    return {
        ...
        "per_beat": {"PR_ms": PR, "QRS_ms": QRS, "QT_ms": QT, "onsets": onsets, "offsets": offsets, "t_end": tend, "p_onset": pons},
