...
try:
    ...
    def segment_layout(gray, layout="3x4", bbox=None):
        ...
        for r in range(rows):
            for c in range(cols):
                lx0 = x0 + int(c*W/cols); lx1 = x0 + int((c+1)*W/cols)
                ly0 = y0 + int(r*H/rows); ly1 = y0 + int((r+1)*H/rows)
                out.append({"lead":LEADS[r][c], "bbox":(lx0,ly0,lx1,ly1)})
        return out

def draw_overlay(image_path: str,
                 leads_boxes: Dict[str, Tuple[int,int,int,int]] | None,
                 rpeaks_map: Dict[str, List[int]] | None,
                 intervals: Dict | None,
                 out_path: str,
                 highlight_lead: str = "II") -> str:
    ...
    if leads_boxes: lab2box.update(leads_boxes)
