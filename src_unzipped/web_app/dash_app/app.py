...
def process(
    n_process, n_rrob, n_intervals, content, filename, meta_text, ops, layout, lead_selected
):
    if not content:
    ...
    if run_rrob or run_intervals:
        if target_lead in lab2box:
            ...
            if run_rrob:
                summary.append(
                    f"R-peaks robustos ({target_lead}): {len(rpeaks_data['peaks_idx'])} picos (fs≈{pxsec:.1f} px/s)"
                )
            if run_intervals:
                iv = intervals_from_trace(trace, rpeaks_data["peaks_idx"], pxsec)
                med = iv["median"]
                summary.append(
                    "PR {PR_ms} ms | QRS {QRS_ms} ms | QT {QT_ms} ms | QTcB {QTc_B} ms | QTcF {QTc_F} ms".format(
                        **med
                    )
                )
        else:
